...
const steps = [
    ...
    { id: 2, label: "Your Details", estimatedTime: "3 min" },
                    ...
                    <div key={step.id} className="relative flex flex-col items-start flex-1">
                        ...
                        {(getStepStatus(step.id) === "current" || getStepStatus(step.id) != "completed") && (
                            <motion.p
                                ...
                                className="text-xs text-gray-400 mt-2"
                            >
                                ( Approx {step.estimatedTime})
                            </motion.p>
                        )}

                        {index < steps.length - 1 && (
                            <div className="absolute top-[47px] left-[20%] w-full h-[2px] pl-2 pr-8">
                                <div className={`h-full transition-all duration-500
                                    ${getStepStatus(step.id) === "completed"
                                        ? "bg-blue-500 border-blue-500 pr-2"
                                        : getStepStatus(step.id) === "current"
                                            ? "border-[1px] border-dashed border-blue-500"
                                            : "border-[1px] border-blue-200 pr-2"}`}
                                />
                            </div>
                        )}
                    </div>
